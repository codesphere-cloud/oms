...
func realConfigManager(writes map[string][]byte) installer.InstallConfigManager {
	icm, err := installer.NewInstallConfigManager("plain", "")
...
func (f *recordingFileIO) WriteFile(path string, content []byte, _ os.FileMode) error {
	f.writes[path] = content
	return nil
}

func (f *recordingFileIO) ReadFile(path string) ([]byte, error) {
...
func (f *recordingFileIO) Exists(path string) bool {
	_, ok := f.writes[path]

	return ok
}

func (f *recordingFileIO) MkdirAll(string, os.FileMode) error {
	return nil
}

// CreateTemp, Rename and Remove keep the vault's atomic write in memory: it writes a temporary
// file next to the target and renames it over it.
func (f *recordingFileIO) CreateTemp(dir, pattern string) (string, error) {
	f.temps++

	return filepath.Join(dir, fmt.Sprintf("%s.%d", pattern, f.temps)), nil
}

func (f *recordingFileIO) Rename(oldPath, newPath string) error {
	content, ok := f.writes[oldPath]
	if !ok {
		return fs.ErrNotExist
	}

	f.writes[newPath] = content
	delete(f.writes, oldPath)

	return nil
}

func (f *recordingFileIO) Remove(path string) error {
...
var _ = Describe("Multi-DC bootstrap", func() {
	...
	JustBeforeEach(func() {
		...
		// The primary data center reuses the bootstrapper's config manager; every other one
		// needs its own, so seed the layout before Bootstrap derives it.
		bs.Env.DataCenters = gcp.BuildDataCenters(bs.Env)
		for _, dc := range bs.Env.DataCenters[1:] {
			dc.ConfigManager = realConfigManager(writes)
		}
	})
	...
	expectBootstrapMocks := func(projectID string) {
		...
		// OpenFGA database backups: service account, storage role, bucket and HMAC key.
		openfgaSA := "openfga-backup@" + projectID + ".iam.gserviceaccount.com"
		gc.EXPECT().CreateServiceAccount(projectID, "openfga-backup", "openfga-backup").Return(openfgaSA, true, nil)
