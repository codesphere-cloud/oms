	...
	Describe("InstallCodesphere", func() {
		...
		Describe("Valid InstallCodesphere", func() {
			Context("Direct GitHub access", func() {
				...
				It("downloads and installs lite package", func() {
					...
					nodeClient.EXPECT().RunCommand(mock.MatchedBy(jumpboxMatcher), "root", "OMS_PORTAL_API_KEY= OMS_PORTAL_API= oms download package -f installer-lite.tar.gz -H abc1234567890 v1.2.3").Return(nil)
			...
			Context("without explicit hash", func() {
				...
				It("downloads and installs codesphere", func() {
					// Expect download package
					nodeClient.EXPECT().RunCommand(mock.MatchedBy(jumpboxMatcher), "root", "OMS_PORTAL_API_KEY= OMS_PORTAL_API= oms download package -f installer.tar.gz -H def9876543210 v1.2.3").Return(nil)
			...
			It("downloads and installs codesphere with hash", func() {
				nodeClient.EXPECT().RunCommand(mock.MatchedBy(jumpboxMatcher), "root", "OMS_PORTAL_API_KEY= OMS_PORTAL_API= oms download package -f installer.tar.gz -H abc1234567890 v1.2.3").Return(nil)
		...
		Describe("Invalid cases", func() {
			...
			It("fails when download package fails", func() {
				nodeClient.EXPECT().RunCommand(mock.MatchedBy(jumpboxMatcher), "root", "OMS_PORTAL_API_KEY= OMS_PORTAL_API= oms download package -f installer.tar.gz -H abc1234567890 v1.2.3").Return(fmt.Errorf("download error"))
			...
			It("fails when install codesphere fails", func() {
				nodeClient.EXPECT().RunCommand(mock.MatchedBy(jumpboxMatcher), "root", "OMS_PORTAL_API_KEY= OMS_PORTAL_API= oms download package -f installer.tar.gz -H abc1234567890 v1.2.3").Return(nil).Once()
