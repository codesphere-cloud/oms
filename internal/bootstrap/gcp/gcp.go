...
func (b *GCPBootstrapper) EnsureIAMRoles() error {
	err := b.ensureIAMRoleWithRetry(b.Env.ProjectID, "cloud-controller", []string{"roles/compute.admin"})
	if err != nil {
		return err
	}
	...
	err = b.ensureIAMRoleWithRetry(b.Env.ProjectID, "artifact-registry-writer", []string{"roles/artifactregistry.writer"})
	return err
}

func (b *GCPBootstrapper) ensureIAMRoleWithRetry(projectID string, serviceAccount string, roles []string) error {
	var err error
	for retries := range 5 {
		err = b.GCPClient.AssignIAMRole(projectID, serviceAccount, roles)
		if err == nil {
			return nil
		}
		if retries < 4 {
...
func (b *GCPBootstrapper) ensureDnsPermissions() error {
	...
	err := b.ensureIAMRoleWithRetry(b.Env.ProjectID, "cloud-controller", []string{"roles/dns.admin"})
	if err != nil {
		return err
	}
	return nil
