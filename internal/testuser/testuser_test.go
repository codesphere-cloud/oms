...
var _ = Describe("createTestUserInDB", func() {
	const (
	...
	It("creates a test user successfully", func() {
		...
		result, err := createTestUserInDB(sqlDB, hashedPassword, hashedToken)
		Expect(err).NotTo(HaveOccurred())
	...
	It("returns an error when the test user already exists", func() {
		...
		_, err = createTestUserInDB(sqlDB, hashedPassword, hashedToken)
