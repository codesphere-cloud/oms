...
// Err aggregates a test run into a single error, or returns nil if every test
// passed. Skipped tests count against the run: a run cut short by its deadline
// did not verify the installation, so it must not pass as if it had.
func Err(results []Result) error {
	...
	var problems []string
	if len(failed) > 0 {
		problems = append(problems, fmt.Sprintf("%d of %d test(s) failed: %s", len(failed), len(results), strings.Join(failed, ",")))
	}
	if len(skipped) > 0 {
		problems = append(problems, fmt.Sprintf("%d of %d test(s) skipped: %s", len(skipped), len(results), strings.Join(skipped, ",")))
	}
