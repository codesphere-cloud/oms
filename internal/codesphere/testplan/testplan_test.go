...
var _ = Describe("Testplan", func() {
	...
	Describe("Err", func() {
		It("returns nil if every test passed", func() {
			Expect(testplan.Err([]testplan.Result{
				{Name: "passes", Status: testplan.StatusPassed},
				{Name: "second", Status: testplan.StatusPassed},
			})).To(BeNil())
		})

		It("counts skipped tests against the run", func() {
			err := testplan.Err([]testplan.Result{
				{Name: "passes", Status: testplan.StatusPassed},
				{Name: "second", Status: testplan.StatusSkipped},
			})

			Expect(err).To(MatchError("1 of 2 test(s) skipped: second"))
		})

		It("names both the failed and the skipped tests", func() {
			err := testplan.Err([]testplan.Result{
				{Name: "fails", Status: testplan.StatusFailed},
				{Name: "second", Status: testplan.StatusSkipped},
			})

			Expect(err).To(MatchError("1 of 2 test(s) failed: fails; 1 of 2 test(s) skipped: second"))
		})

		It("fails a run whose deadline expired before any test ran", func() {
			ctx, cancel := context.WithTimeout(context.Background(), 0)
			defer cancel()

			results := (&testplan.Runner{Out: out}).Run(ctx, []testplan.Test{passes, second})

			Expect(ran).To(BeEmpty())
			Expect(testplan.Err(results)).To(MatchError("2 of 2 test(s) skipped: passes,second"))
		})
