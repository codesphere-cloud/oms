...
// autoVault selects the backend that matches the vault file on disk. The bootstrap
// flows use it because they accept a vault that is either plaintext or
// SOPS-encrypted. The format is detected on every Load and Save, so a vault that
// changes format between runs (for example after --recover-config downloads a
// decrypted copy) keeps working.
type autoVault struct{ options Options }

// DetectType reports the on-disk vault type of path. A missing file is reported as
// TypePlain so callers can create a new vault, and any other read failure is
...
func (v *autoVault) LoadOrCreate() (*files.InstallVault, error) {
	...
	loaded, err := backend.LoadOrCreate()
	if err != nil {
		return nil, fmt.Errorf("failed to load vault %s: %w", v.options.Path, err)
	}
...
// Save writes the vault in the format that is already on disk, so a SOPS-encrypted
// vault stays encrypted and a plaintext one stays plaintext. A vault that does not
// exist yet is written as plaintext, matching the pre-existing bootstrap behaviour
// of encrypting it on the jumpbox instead.
