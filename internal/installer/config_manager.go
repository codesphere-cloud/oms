...
func (g *InstallConfig) ValidateVault() []string {
	...
	// Generated by EnsureSecrets for a fresh installation, but a data center joining an
	// existing installation must carry over the primary data center's key — the services
	// and the OpenFGA instance they share authenticate with the same one.
	if !foundSecrets[files.SecretOpenFgaPresharedKey] {
		errors = append(errors, fmt.Sprintf(
			"required secret missing: %s (in a multi-data-center installation, copy it from the data center that deploys OpenFGA)",
