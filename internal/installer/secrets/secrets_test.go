...
var _ = Describe("EnsureOpenFgaPresharedKey", func() {
	...
	It("is shared across data centers", func() {
		vault := newVault()
		Expect(secrets.EnsureOpenFgaPresharedKey(vault)).To(Succeed())
		primary := vault.GetSecret("openFgaPresharedKey").Fields.Password

		derived := secrets.DeriveDataCenterVault(vault)
		Expect(secrets.EnsureOpenFgaPresharedKey(derived)).To(Succeed())

		Expect(derived.GetSecret("openFgaPresharedKey").Fields.Password).To(Equal(primary))
	})
})
