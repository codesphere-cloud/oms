...
```
      ...
      --age-key string                             Path to the age private key used to encrypt the vault (falls back to SOPS_AGE_KEY or SOPS_AGE_KEY_FILE)
  ...
  -h, --help                                       help for install-config
      ...
      --vault-type string                          Vault storage type (sops or plain), without an age key a sops vault is written unencrypted (default "sops")
