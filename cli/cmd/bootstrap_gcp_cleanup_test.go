...
var _ = Describe("BootstrapGcpCleanupCmd", func() {
	...
	Describe("executeCleanup", func() {
		...
		Context("when external DNS project is configured", func() {
			It("should revoke impersonation permissions in DNS project", func() {
				cleanupCmd.Opts.ProjectID = "test-project"
				cleanupCmd.Opts.Force = true
				...
				validEnv := gcp.CodesphereEnvironment{
					...
					DNSZoneName:              "test-zone",
					DNSProjectID:             "dns-project",
					DNSProjectServiceAccount: "dns-sa",
				}
				envData, _ := json.Marshal(validEnv)

				mockFileIO.EXPECT().Exists("/tmp/test-infra.json").Return(true)
				mockFileIO.EXPECT().ReadFile("/tmp/test-infra.json").Return(envData, nil)
				mockGCPClient.EXPECT().DeleteDNSRecordSets("dns-project", "test-zone", "example.com").Return(nil)
				mockGCPClient.EXPECT().RevokeImpersonation("cloud-controller", "test-project", "dns-sa", "dns-project").Return(nil)
				mockGCPClient.EXPECT().DeleteProject("test-project").Return(nil)
				mockFileIO.EXPECT().Exists("/tmp/test-infra.json").Return(true)
			...
			It("should continue cleanup even if revoking impersonation fails", func() {
				cleanupCmd.Opts.ProjectID = "test-project"
				cleanupCmd.Opts.Force = true
				...
				mockFileIO.EXPECT().ReadFile("/tmp/test-infra.json").Return(envData, nil)
				mockGCPClient.EXPECT().DeleteDNSRecordSets("dns-project", "test-zone", "example.com").Return(nil)
				mockGCPClient.EXPECT().RevokeImpersonation("cloud-controller", "test-project", "dns-sa", "dns-project").Return(errors.New("permission denied"))
				mockGCPClient.EXPECT().DeleteProject("test-project").Return(nil)
				mockFileIO.EXPECT().Exists("/tmp/test-infra.json").Return(true)
			...
			It("should not revoke impersonation if DNS project is the same as main project", func() {
				cleanupCmd.Opts.ProjectID = "test-project"
				cleanupCmd.Opts.Force = true
			...
			It("should not revoke impersonation if DNS service account is empty", func() {
				cleanupCmd.Opts.ProjectID = "test-project"
				cleanupCmd.Opts.Force = true
