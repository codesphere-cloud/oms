...
func AddBootstrapLocalCmd(parent *cobra.Command) {
	...
	flags.StringVar(&bootstrapLocalCmd.CodesphereEnv.ArgoCDRegistryURL, "registry-url", "ghcr.io/codesphere-cloud/charts", "OCI registry URL used for the ArgoCD helm pull secret (only relevant with --argocd)")
	flags.StringVar(&bootstrapLocalCmd.CodesphereEnv.ArgoCDVaultFile, "vault-file", "", "Path to the SOPS-encrypted vault file to deploy as a Kubernetes secret (only relevant with --argocd)")
	flags.StringVar(&bootstrapLocalCmd.CodesphereEnv.ArgoCDVaultNamespace, "vault-namespace", argocd.DefaultVaultNamespace, "Kubernetes namespace for the vault secret (only relevant with --argocd)")
	flags.StringVar(&bootstrapLocalCmd.CodesphereEnv.ArgoCDVaultSecretName, "vault-secret-name", argocd.DefaultVaultSecretName, "Name of the Kubernetes secret created from the vault (only relevant with --argocd)")
