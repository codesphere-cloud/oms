...
var _ = Describe("BuildImagesCmd", func() {
	...
	Context("BuildAndPushImages method", func() {
		It("fails when config manager fails to parse config", func() {
			...
			err := c.BuildAndPushImages(mockConfigManager, mockImageManager)
			Expect(err).To(HaveOccurred())
		...
		It("fails when image build fails", func() {
			...
			mockImageManager.EXPECT().BuildImage("Dockerfile", "registry.example.com/my-ubuntu-24.04-default:0.0.0", ".").Return(errors.New("build failed"))
		...
		It("fails when image push fails", func() {
			...
			mockConfigManager.EXPECT().ParseConfigYaml("config-with-dockerfile.yaml").Return(configWithDockerfile, nil)
			mockImageManager.EXPECT().BuildImage("Dockerfile", "registry.example.com/my-ubuntu-24.04-default:0.0.0", ".").Return(nil)
			mockImageManager.EXPECT().PushImage("registry.example.com/my-ubuntu-24.04-default:0.0.0").Return(errors.New("push failed"))
		...
		It("successfully builds and pushes single image", func() {
			...
			mockConfigManager.EXPECT().ParseConfigYaml("config-with-dockerfile.yaml").Return(configWithDockerfile, nil)
			mockImageManager.EXPECT().BuildImage("Dockerfile", "registry.example.com/my-ubuntu-24.04-default:0.0.0", ".").Return(nil)
			mockImageManager.EXPECT().PushImage("registry.example.com/my-ubuntu-24.04-default:0.0.0").Return(nil)
		...
		It("successfully builds and pushes multiple images with different flavors", func() {
			...
			mockImageManager.EXPECT().BuildImage("Dockerfile.default", "registry.example.com/my-ubuntu-24.04-default:0.0.0", ".").Return(nil)
			mockImageManager.EXPECT().PushImage("registry.example.com/my-ubuntu-24.04-default:0.0.0").Return(nil)

			// Expect calls for my-ubuntu-24.04 minimal flavor
			mockImageManager.EXPECT().BuildImage("Dockerfile.minimal", "registry.example.com/my-ubuntu-24.04-minimal:0.0.0", ".").Return(nil)
			mockImageManager.EXPECT().PushImage("registry.example.com/my-ubuntu-24.04-minimal:0.0.0").Return(nil)

			// Expect calls for my-alpine-3.18 default flavor
			mockImageManager.EXPECT().BuildImage("Dockerfile.alpine", "registry.example.com/my-alpine-3.18-default:0.0.0", ".").Return(nil)
			mockImageManager.EXPECT().PushImage("registry.example.com/my-alpine-3.18-default:0.0.0").Return(nil)
